...
- Web App 工作已移至 `LockedCV-APP`。See `plan.authenticated-sessions.md` in the App repo.
...
5. **Web App repo handoff**：Web App work has moved to `LockedCV-APP`. See `plan.authenticated-sessions.md` in the App repo.
...
6. ✅ `web-app-repo-handoff`（已完成）
   ...
   - See `plan.authenticated-sessions.md` in the App repo.
   - API repo keeps API contract, auth route, authorization demo, API tests, and schema docs.

## 依賴順序

- `api-authentication-service` -> `api-authentication-route`（已完成）
- `api-authentication-route` 已完成；App-side API client work is tracked in `LockedCV-APP`.
...
- `web-app-repo-handoff` 已完成；Web App task tracking now lives in `LockedCV-APP/.github/plans/plan.authenticated-sessions.md`.
...
- Web App repo 已建立為 `LockedCV-APP`；部署方式待決。
...
- Web App login/session/flash/UI work is tracked in `LockedCV-APP/.github/plans/plan.authenticated-sessions.md`.
- 至少一段 role-based API guard demo 已完成並有測試。
