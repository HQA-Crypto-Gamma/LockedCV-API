...
- Account model 需要提供 system-level capability summary，例如 admin/system-role-management/resource-create privileges。
...
- 目前尚未有：
  ...
  - `AccountPolicy`、`AttachmentPolicy`、`SensitiveDataPolicy`、`MaskedAttachmentPolicy` 等 centralized policy objects。
  - Policy scope objects，例如 current account 可 list 的 attachments/accounts。
  - `summary` / `index_summary` response contract。
  - Account model 的 `capabilities` / system-level capability method。
...
2. **Policy object foundation**：建立 `app/policies/`，先做 `AccountPolicy` 與 `AttachmentPolicy`，覆蓋目前最常用 resource。
3. **Policy summaries**：每個 policy 提供 `summary`；需要 index response 時提供 `index_summary`。
4. **Policy scopes**：建立 list/index routes 用的 scope object，例如 `AttachmentPolicy::AccountScope#viewable`、`AccountPolicy::AccountScope#viewable`。
...
6. **Account capabilities**：在 Account model 或 AccountPolicy 建立 system-level capability summary，回傳目前 account 可以做的 system actions。
...
5. `policy-scopes`
   ...
   - 若 admin settings 需要，建立 `AccountPolicy::AccountScope#viewable` 作為 admin account list。
   - 補 scope/policy consistency tests：scope 回傳的每個 resource 都應 `can_view? == true`。

6. `controller-policy-adoption`
